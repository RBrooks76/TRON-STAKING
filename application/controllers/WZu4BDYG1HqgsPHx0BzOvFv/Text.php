<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Text extends Admin_Controller {

	/**
	 * Index Page for this controller.
	 *
	 * Maps to the following URL
	 * 		http://example.com/index.php/welcome
	 *	- or -
	 * 		http://example.com/index.php/welcome/index
	 *	- or -
	 * Since this controller is set as the default controller in
	 * config/routes.php, it's displayed at http://example.com/
	 *
	 * So any other public methods not prefixed with an underscore will
	 * map to /index.php/welcome/<method_name>
	 * @see https://codeigniter.com/user_guide/general/urls.html
	 */

	public function index()
	{
		echo "Gfgf";
	}


	function textmanage() 
	{
		if(empty(admin_id())) 
		{
			admin_url_redirect('', 'refresh');
		}

		user_access();

		$user_view = $this->config->item('user_view');


		if(!in_array('14',$user_view))
		{
			admin_redirect('admindashboard', 'refresh');
		}

			$data['title'] 			= 	'Text Manage';

			$data['text'] 	=	FS::Common()->getTableData(TEXT, '', '', '', '', '', '','', array('id', 'DESC'))->result();

			$this->view('pages/Text/textmanage', $data);
		
	}


	// Edit page
	function edittext($id='') {
		user_access();

		$user_view = $this->config->item('user_view');

		if(!in_array('14',$user_view))
		{
			admin_redirect('admindashboard', 'refresh');
		}

		// Is logged in
		$sessionvar = $this->session->userdata('loggedadmin');

		if (!$sessionvar) {
			admin_url_redirect('', 'refresh');
		}
		// Is valid
		$text_id = insep_decode($id);
		if ($id == '') {
			FS::session()->set_flashdata('error', 'Invalid request');
			admin_redirect('text', 'refresh');

		}
		$isValid = FS::Common()->getTableData(TEXT, array('id' => $text_id));
		if ($isValid->num_rows() == 0) {
			FS::session()->set_flashdata('error', 'Unable to find this page');
			admin_redirect('text', 'refresh');

		}
		// Form validation
		$this->form_validation->set_rules('title', 'title', 'required|xss_clean');
		$this->form_validation->set_rules('content_description', 'content', 'required|xss_clean');
		$this->form_validation->set_rules('status', 'status', 'required|xss_clean');
		$this->form_validation->set_rules('language', 'language', 'required|xss_clean');

		if ($this->input->post()) {
			if ($this->form_validation->run()) {
				$updateData = array();
				$title = escapeString(strip_tags($this->input->post('title')));
				$content = $this->input->post('content_description');
				$status = escapeString(strip_tags($this->input->post('status')));
				$language = escapeString(strip_tags($this->input->post('language')));
				$updateData['title'] = $title;
				$updateData['content'] = $content;
				$updateData['status'] = $status;
				$updateData['language'] = $language;
				$updateData['updated_at'] =  date('Y-m-d H:i:s');;

				$condition = array('id' => $text_id);
				
				$update = FS::Common()->updateTableData(TEXT, $condition, $updateData);
				if ($update) {
					FS::session()->set_flashdata('success', 'Text has been updated successfully!');
					admin_redirect('text', 'refresh');
				} else {
					FS::session()->set_flashdata('error', 'Unable to update this text');
					admin_redirect('textedit/' . $id, 'refresh');
				}

			} else {
				FS::session()->set_flashdata('error', 'Unable to update this text');
				admin_redirect('textedit/' . $id, 'refresh');
			}

		}
			$data['action'] 		= 	base_url() . 'edittext';
			
			$data['title'] 			= 	'Edit text';

			$data['mode'] 			= 	'Edit';

			$data['text'] 	=	FS::Common()->getTableData(TEXT, array('id' => $text_id))->row();
			$data['lang'] 	=	FS::Common()->getTableData(LANG)->result();

			$this->view('pages/Text/edittext', $data);
	}



	function deletetext($id) {

		user_access();

		$user_view = $this->config->item('user_view');

		if(!in_array('14',$user_view))
		{
			admin_redirect('admindashboard', 'refresh');
		}

		// Is logged in
		$sessionvar = $this->session->userdata('loggedadmin');

		if (!$sessionvar) {
			admin_url_redirect('', 'refresh');
		}
		// Is valid
		$text_id = insep_decode($id);

		if ($id == '') {
			$this->session->set_flashdata('error', 'Invalid request');
			admin_redirect('text', 'refresh');
		}
		$isValid = FS::Common()->getTableData(TEXT, array('id' => $text_id))->num_rows();
		if ($isValid > 0) {
			// Check is valid
			$condition = array('id' => $text_id);
			$delete = FS::Common()->deleteTableData(TEXT, $condition);
			
			if ($delete) {
				// True // Delete success
				FS::session()->set_flashdata('success', 'text deleted successfully');
				admin_redirect('text', 'refresh');
			} else {
				//False
				FS::session()->set_flashdata('error', 'Problem occure with text deletion');
					admin_redirect('text', 'refresh');
			}
		} else {
			FS::session()->set_flashdata('error', 'Unable to find this page');
			admin_redirect('text', 'refresh');
		}
	}


	function addtext() {

		user_access();

		$user_view = $this->config->item('user_view');

		if(!in_array('14',$user_view))
		{
			admin_redirect('admindashboard', 'refresh');
		}

		// Is logged in
		$sessionvar = $this->session->userdata('loggedadmin');

		if (!$sessionvar) {
			admin_url_redirect('', 'refresh');
		}
		// Form validation
		$this->form_validation->set_rules('addtitle', 'title', 'required|xss_clean');
		$this->form_validation->set_rules('content_description_add', 'content', 'required');
		$this->form_validation->set_rules('addstatus', 'status', 'required|xss_clean');
		$this->form_validation->set_rules('addlanguage', 'addlanguage', 'required|xss_clean');

		if ($this->input->post()) {
			if ($this->form_validation->run()) {
				$insertData = array();
				
				$insertData['title'] = escapeString(strip_tags($this->input->post('addtitle')));				
				$insertData['content'] = $this->input->post('content_description_add');
				$insertData['status'] = escapeString(strip_tags($this->input->post('addstatus')));
				$insertData['language'] = escapeString(strip_tags($this->input->post('addlanguage')));
				$insertData['created_at'] = strtotime("now");

				$insert = FS::Common()->insertTableData(TEXT, $insertData);
				if ($insert) {
					FS::session()->set_flashdata('success', 'Text has been added successfully!');
				admin_redirect('text', 'refresh');
				} else {
					FS::session()->set_flashdata('error', 'Unable to add the new text !');
				admin_redirect('text', 'refresh');
				}

			} else {
				FS::session()->set_flashdata('error', 'Some data are missing!');
				admin_redirect('text', 'refresh');
			}

		}

		$data['action'] 		= 	base_url() . 'text';
			
		$data['title'] 			= 	'Add text';

		$data['mode'] 			= 	'Add';

		$data['lang'] 	=	FS::Common()->getTableData(LANG)->result();

		$this->view('pages/Text/edittext', $data);

	}
}